 ...
 */
enum cg_upscale_fit {
	/* Largest whole multiple that fits, centred. Reproducible exactly, at
	 * the cost of leaving a border on most panels. */
	CG_UPSCALE_EXACT,
	/* Largest rectangle that keeps the virtual output's aspect ratio,
	 * centred. Fills one axis exactly. */
	CG_UPSCALE_FIT,
	/* Stretched to the whole output. Fills the panel and distorts the
	 * aspect ratio when the two do not agree. */
	CG_UPSCALE_FILL,
};

enum cg_upscale_filter {
	/* Nearest for an exact scale, interpolated otherwise. */
	CG_UPSCALE_FILTER_AUTO,
	CG_UPSCALE_FILTER_NEAREST,
	CG_UPSCALE_FILTER_BILINEAR,
};

struct cg_upscale {
	bool enabled;
	enum cg_upscale_fit fit;
	enum cg_upscale_filter filter;
	int width, height;
